...
function get_latest_stage_tarball_filename() {
	# There is probably a better way to do this, but for now it works :)
	target="${ARCH}"
	[[ "${ARCH}" == "x86" ]] && target="i686"
	data=$(wget -nv -O- "${MIRROR}/releases/${ARCH}/autobuilds/latest-stage3.txt" | grep -v "hardened" | grep -v "nomultilib" | egrep "^[0-9]{8}/stage3-${target}-[0-9]{8}\.tar\.bz2$" )
	if [[ "$?" -ne '0' ]]; then
